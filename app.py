...
st.caption("성간 소광 및 적색화 분석 시뮬레이터")

# 소개글 및 물리적 원리 설명
st.markdown("""
이 프로그램은 지구과학 II 천문 단원의 핵심 개념인 성간 소광과 성간 적색화가 별의 관측 등급 및 거리 측정에 미치는 영향을 실시간으로 시각화하고 정량적으로 분석합니다.

### 🔍 왜 이런 현상이 나타날까요? (물리적 원리)
우주 공간의 성간 티끌은 빛을 흡수하거나 사방으로 흩어지게 만드는 산란 현상을 일으킵니다. 
...
# 6. 메인 대시보드 시각화 (2열 구성 - 깔끔한 화이트 테마 설정)
...
with col2:
    ...
    st.markdown("소광량을 보정하지 않으면 별이 실제보다 어둡게 보여 거리가 비정상적으로 길게 계산됩니다.")
    
    fig_distance = go.Figure()
    fig_distance.add_trace(go.Bar(
        x=df["별 이름"], y=df["실제 거리 (pc)"],
        name="실제 거리", marker_color="#00CCFF"
    ))
    fig_distance.add_trace(go.Bar(
        x=df["별 이름"], y=df["소광 무시 겉보기 거리 (pc)"],
        name="소광 무시 거리", marker_color="#FF3366"
    ))
    ...
    fig_distance.update_layout(
        ...
        xaxis=dict(title="별 이름", gridcolor='rgba(0, 0, 0, 0.1)'),
        yaxis=dict(title="거리 (pc)", gridcolor='rgba(0, 0, 0, 0.1)'),
        height=450
    )
...
with col_info1:
    st.markdown("""
    ### 🎯 탐구 목적
    1. 다양한 천체 데이터 분석: 고유 색지수, 절대 등급, 실제 거리 등 다양한 속성을 가진 천체 데이터를 연계 처리하여 다각적인 데이터 분석 역량을 기릅니다.
...
with col_info2:
    st.markdown("""
    ### 📖 지구과학 II 핵심 개념
    - 성간 소광: 성간 티끌이 빛을 흡수·산란시켜 별이 본래보다 어둡게 보이는 현상입니다. (겉보기 등급이 증가함)
    - 성간 적색화 및 색초과: 푸른빛이 더 강하게 산란되므로 관측 색지수가 고유 색지수보다 커져 별이 붉게 보입니다. 
      $$E = (B-V) - (B-V)_0$$
    - 거리 측정 오차 공식: 소광량을 누락하면 거리지수 공식에 의해 실제 거리보다 더 멀리 있는 것으로 오인하게 됩니다.
      $$V - M = 5\\log_{10}d - 5$$
    """)
