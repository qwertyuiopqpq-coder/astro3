...
# [수정] 소개글에 성간 적색화와 소광이 일어나는 물리적 원리(레일리 산란 등) 추가
st.markdown("""
이 프로그램은 지구과학 II 천문 단원의 핵심 개념인 **성간 소광(Stellar Extinction)**과 **성간 적색화(Reddening)**가 별의 관측 등급 및 거리 측정에 미치는 영향을 실시간으로 시각화합니다.

### 🔍 왜 이런 현상이 나타날까요? (물리적 원리)
우주 공간의 **성간 티끌(Dust)**은 빛을 흡수하거나 사방으로 흩어지게 만드는 **산란(Scattering)** 현상을 일으킵니다. 
이때 파장이 긴 붉은빛보다 **파장이 짧은 푸른빛이 훨씬 더 강하게 산란**됩니다. 
결과적으로 지구에 도달하는 별빛은 푸른빛을 많이 잃어 **본래보다 어둡게 보이고(성간 소광)**, **붉은색을 띠게 됩니다(성간 적색화)**.
""")
st.markdown("---")
...
with col2:
    ...
    st.markdown("소광량($A_V$)을 무시하면 별이 실제보다 더 어둡게 보여 거리가 멀게 측정됩니다.")
...
with col_info1:
    # [수정] 탐구 목적에 다양한 천체 데이터 분석 및 활용 관련 내용 추가
    st.markdown("""
    ### 🎯 탐구 목적
...
with col_info2:
    ...
    - **성간 적색화 & 색초과 ($E_{B-V}$):** 푸른빛이 더 강하게 산란되므로 관측 색지수가 고유 색지수보다 커져 별이 붉게 보입니다. 
      $$E_{B-V} = (B-V) - (B-V)_0$$
    - **거리 측정 오차 공식:** 소광량($A_V$)을 누락하면 거리지수 공식($V - M_V = 5\log_{10}d - 5$)에 의해 **실제 거리보다 더 멀리 있는 것으로 오인**하게 됩니다.
    """)
